...
function saveTeacherAccount(sheet, teacherData) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (data[i][0] === teacher.id) {
      teacherRow = i + 1; // +1 because sheet rows are 1-indexed
      break;
    }
  }
  ...
  if (teacherRow > 0) {
    ...
    sheet.getRange(teacherRow, 2).setValue(teacher.username);
    sheet.getRange(teacherRow, 3).setValue(teacher.name);
    sheet.getRange(teacherRow, 4).setValue(teacher.class);
    sheet.getRange(teacherRow, 5).setValue(teacher.active);
    
    // Only update password if provided
    if (teacher.password) {
      sheet.getRange(teacherRow, 6).setValue(teacher.password);
    }
  } else {
    // Add new teacher
    sheet.appendRow([
...
function deleteTeacherAccount(sheet, teacherId) {
  var data = sheet.getDataRange().getValues();
  var teacherRow = -1;
...
function getAllTeacherAccounts(sheet) {
  var data = sheet.getDataRange().getValues();
  var teachers = [];
...
function handleTeacherLogin(sheet, username, password) {
  var data = sheet.getDataRange().getValues();
  
  for (var i = 1; i < data.length; i++) {
    if (data[i][1] === username && data[i][5] === password && data[i][4] === true) {
...
// Function to create TeacherAccounts sheet with default structure
function createTeacherAccountsSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheet = ss.insertSheet('TeacherAccounts');
  ...
  sheet.setColumnWidth(3, 150); // Name
  sheet.setColumnWidth(4, 100); // Class
  sheet.setColumnWidth(5, 80);  // Active
  sheet.setColumnWidth(6, 200); // Password (hashed)
  sheet.setColumnWidth(7, 180); // LastLogin
